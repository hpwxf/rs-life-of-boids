...
use crate::glx::{self, ShaderProgram, VertexArray, Buffer};
...
impl<'a> Renderer<'a> {
    pub fn new(gl: &gl::Gl, config: RendererConfig) -> Renderer {
        let program = ShaderProgram::new(gl, VS_SRC, FS_SRC).expect("Problem creating shader program");
